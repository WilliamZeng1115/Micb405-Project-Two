...
setwd("~")
KO_log <- read_table2("Desktop/MICB_405/Project2/Nitrogen_Sulfur/KO_log.csv", 
                      col_names = TRUE) %>%
  .[,-2]%>%
  .[,-(4:5)] %>%
...
for (x in 1:length(S_cycle_cruise$KEGG)) {
  S_cycle_cruise$Gene[x]=KO_log$Nitrogen_1[S_cycle_cruise$KEGG[x]==(KO_log$`00910`[65:166])]
}
